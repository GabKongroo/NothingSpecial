...
from model import SessionLocal, Beat, Bundle, BundleBeat, Order, BundleOrder
...
# Gestione sicura di SessionLocal
def get_session():
    """Ottiene una sessione database sicura"""
    if SessionLocal is None:
        # Crea SessionLocal al volo se non esiste
        database_url = os.environ.get("DATABASE_URL")
        if not database_url:
            raise RuntimeError("DATABASE_URL non configurato")
        engine = create_engine(database_url)
        SessionLocal_temp = sessionmaker(bind=engine)
        return SessionLocal_temp()
    return SessionLocal()
        ...
        try:
            with SessionLocal() as db:
                ...
                new_bundle = Bundle(
                    ...
                    bundle_price=float(request.form.get("bundle_price", 0))
                )
                db.add(new_bundle)
                db.commit()
        ...
        try:
            with SessionLocal() as db:
                ...
                if bundle:
                    ...
                    bundle.individual_price = float(request.form.get("individual_price", 0))
                    bundle.bundle_price = float(request.form.get("bundle_price", 0))
                    db.commit()
                    return redirect(url_for("bundles"))
        except Exception as e:
...
def init_database():
    """Inizializza completamente il database (DANGER ZONE!)"""
    if not session.get("logged_in"):
        return jsonify({"error": "Non autorizzato"}), 401
    ...
    try:
        ...
        database_url = os.environ.get("DATABASE_URL")
        if not database_url:
            return jsonify({"error": "DATABASE_URL non configurato"}), 500
        ...
        # Ricrea tutte le tabelle con struttura corretta
        from model import Base
        Base.metadata.create_all(engine)
        
        return jsonify({
            "success": True, 
            "message": f"Database inizializzato! Eliminate {len(tables)} tabelle e ricreate con struttura corretta."
        })
        
    except Exception as e:
        print(f"❌ Errore inizializzazione database: {e}")
        return jsonify({"error": f"Errore durante l'inizializzazione: {str(e)}"}), 500
