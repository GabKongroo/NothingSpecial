...
class BeatManager {
  ...
  initializeMobileControls() {
    ...
    console.log('🎯 Has main-admin-page class:', document.body.classList.contains('main-admin-page'));
    console.log('🔍 Existing mobile controls:', document.querySelector('.mobile-controls'));
    
    // Only add mobile controls if we're on mobile AND on the main admin page
    if (window.innerWidth <= 768 && document.body.classList.contains('main-admin-page')) {
      console.log('✅ Adding mobile controls');
      this.addMobileControls();
    } else {
      console.log('❌ Not adding mobile controls - conditions not met');
    }
    
    // Handle window resize
    window.addEventListener('resize', () => {
      console.log('📱 Window resized, width:', window.innerWidth);
      if (window.innerWidth <= 768 && !document.querySelector('.mobile-controls') && document.body.classList.contains('main-admin-page')) {
        console.log('✅ Adding mobile controls on resize');
        this.addMobileControls();
      } else if (window.innerWidth > 768 || !document.body.classList.contains('main-admin-page')) {
        console.log('❌ Removing mobile controls on resize');
  ...
  addMobileControls() {
    console.log('🔧 addMobileControls called');
    if (document.querySelector('.mobile-controls')) {
      console.log('⚠️ Mobile controls already exist, skipping');
      return;
    }
  ...
  removeMobileControls() {
    console.log('🗑️ removeMobileControls called');
    const controls = document.querySelector('.mobile-controls');
    if (controls) {
      console.log('✅ Removing mobile controls from DOM');
      controls.remove();
    } else {
      console.log('⚠️ No mobile controls found to remove');
    }
  }
...
class ImageUploadManager {
  constructor() {
    ...
    this.currentImageKey = null; // Track the current uploaded image
    this.handlersInitialized = false; // Prevent duplicate handlers
    this.lastUploadedFileId = null; // Prevent duplicate uploads of same file
    this.lastClickTime = 0; // Prevent rapid clicks
    this.initializeUploadHandlers();
    this.initializeNavigationHandlers();
  }

  initializeNavigationHandlers() {
    // Handle page navigation away - cleanup uploaded image if not saved
    window.addEventListener('beforeunload', (e) => {
      if (this.currentImageKey && !this.isFormSubmitted) {
        // Cleanup image in background
    ...
    // Track form submission to avoid cleanup
    ...
    const cancelButtons = document.querySelectorAll('a[href*="bundle"], button[onclick*="bundle"]');
  ...
  cleanupCurrentImage() {
    if (!this.currentImageKey) return;

    console.log('🧹 Cleaning up uploaded image:', this.currentImageKey);
    
    // Send cleanup request in background
    fetch('/api/cleanup-bundle-image', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ 
        image_key: this.currentImageKey 
      }),
  ...
  initializeUploadHandlers() {
    ...
    if (this.handlersInitialized) {
      console.log('⚠️ Upload handlers already initialized, skipping');
      return;
    }
    ...
    // Click to upload - use arrow function to preserve 'this'
    const handleClick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      ...
      console.log('📸 Upload area clicked, isUploading:', this.isUploading, 'timeSinceLastClick:', timeSinceLastClick);
      
      // Prevent rapid clicks (less than 1 second)
      if (timeSinceLastClick < 1000) {
        console.log('⚠️ Click too rapid, ignoring');
        return;
      }
    ...
    uploadArea.addEventListener('click', handleClick, { once: false, passive: false });
    ...
    // File input change - with stronger duplicate protection
    const handleFileChange = (e) => {
      console.log('📁 File input changed, files:', e.target.files.length);
      
      if (this.isUploading) {
        console.log('⚠️ Upload already in progress, ignoring file change');
        return;
      }
  ...
  async handleFileUpload(file) {
    ...
    // Prevent multiple uploads with stronger check
    if (this.isUploading) {
      console.log('⚠️ Upload already in progress, ignoring...');
      toastManager.show('Caricamento già in corso, attendi...', 'warning');
      return;
    }

    // Additional protection against duplicate uploads of the same file
    const fileId = `${file.name}_${file.size}_${file.lastModified}`;
    if (this.lastUploadedFileId === fileId) {
      console.log('⚠️ Same file already being processed, ignoring...');
      return;
    }
    
    this.lastUploadedFileId = fileId;
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('📸 Image key:', result.image_key);
        console.log('🔗 Image URL:', result.image_url);
        
        // Update the image key field
        if (imageKeyInput) {
          imageKeyInput.value = result.image_key;
          console.log('✅ Updated image key input field');
        } else {
          console.warn('⚠️ Image key input field not found');
        }

        // Track current image for cleanup
        this.currentImageKey = result.image_key;
        console.log('✅ Tracked current image for cleanup');

        // Show preview
        console.log('🖼️ Calling showImagePreview with URL:', result.image_url);
      ...
      // Se c'è stato un errore dopo l'upload, prova a pulire l'immagine
      const imageKeyInput = document.querySelector('input[name="image_key"]');
      if (imageKeyInput && imageKeyInput.value) {
        console.log('Attempting to cleanup image after upload error:', imageKeyInput.value);
        try {
          fetch('/api/cleanup-bundle-image', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ image_key: imageKeyInput.value })
          }).then(response => {
            if (response.ok) {
              console.log('Successfully cleaned up image after upload error');
            } else {
              console.warn('Failed to cleanup image after upload error');
            }
          }).catch(cleanupError => {
            console.error('Error during cleanup after upload error:', cleanupError);
          });
        } catch (cleanupError) {
          console.error('Error during cleanup after upload error:', cleanupError);
        }
        
        // Reset the image key field
        imageKeyInput.value = '';
      }
      ...
      this.isUploading = false;
      this.lastUploadedFileId = null; // Reset to allow re-uploading same file
  ...
  showImagePreview(imageUrl) {
    console.log('🖼️ showImagePreview called with URL:', imageUrl);
    const uploadArea = document.getElementById('image-upload-area');
    ...
    console.log('✅ Upload area found, updating with image preview');
...
// Global function to ensure mobile controls are properly managed
function cleanupMobileControls() {
  console.log('🧹 Global cleanup of mobile controls');
  const controls = document.querySelector('.mobile-controls');
  if (controls && !document.body.classList.contains('main-admin-page')) {
    console.log('✅ Removing mobile controls from non-main page');
    controls.remove();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Always clean up mobile controls first
  cleanupMobileControls();
  
  // Only initialize BeatManager on the main admin page
  if (document.body.classList.contains('main-admin-page')) {
    window.beatManager = new BeatManager();
    console.log('✅ BeatManager instance created:', window.beatManager);
  } else {
    console.log('ℹ️ BeatManager not initialized (not on main admin page)');
    // Extra cleanup for non-main pages
    cleanupMobileControls();
  }
  
  // Always initialize ImageUploadManager for bundle pages
  if (!window.imageUploadManager) {
    window.imageUploadManager = new ImageUploadManager();
    console.log('✅ ImageUploadManager initialized');
  } else {
    console.log('⚠️ ImageUploadManager already exists, skipping initialization');
  }
  
  // Add listener for "Save All" button (both desktop and mobile) - only on main admin page
  if (document.body.classList.contains('main-admin-page')) {
  ...
  // Add some helpful keyboard shortcuts - only on main admin page
  if (document.body.classList.contains('main-admin-page')) {
    ...
    // Only escape to clear toasts on other pages
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        toastManager.clear();
      }
